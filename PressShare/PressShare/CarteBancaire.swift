...
func getAllCard(userId:Int, completionHandlerCards: @escaping (_ success: Bool, _ cardArray: [[String:AnyObject]]?, _ errorString: String?) -> Void) {
    ...
    let body: String =
    "VERSION=00104&TYPE=00001&SITE=1999888&RANG=032&CLE=1999888I&NUMQUESTION=194102418&MONTANT=1000&DEVISE=978&REFABONNE=fxpast@gmail.com&REFERENCE=TestPayb ox&PORTEUR=1111222233334444&DATEVAL=1216&CVV=123&ACTIVITE=024& DATEQ=30012013&PAYS="
    ...
    let task = URLSession.shared.dataTask(with: request as URLRequest) { data, response, error in
        ...
        guard let statusCode = (response as? HTTPURLResponse)?.statusCode , statusCode >= 200 && statusCode <= 299 else {
            completionHandlerCards(false, nil, "Your request returned a status code other than 2xx! : \(StatusCode(((response as? HTTPURLResponse)?.statusCode)!))")
            return
        }
