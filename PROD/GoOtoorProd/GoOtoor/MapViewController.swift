...
//
//  Update by MacbookPRV on 19/03/2018.
...
class MapViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate, UITextFieldDelegate {
    ...
    @objc private func routineTimer() {
        ...
        if config.isTimer == false {
            MyTools.sharedInstance.checkBadge(menuBar: tabBarController)
        }
    ...
    @objc private func pushProduct() {
        ...
        MyTools.sharedInstance.pushProduct(menuBar: tabBarController) { (success, product, errorStr) in
    ...
    @IBAction func actionHelp(_ sender: Any) {
        ...
        //action info
        MyTools.sharedInstance.showHelp("carte_liste", self)
