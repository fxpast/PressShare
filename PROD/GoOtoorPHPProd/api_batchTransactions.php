...
//---------------------------------------------------------------------------------------
//update time slots start date and due date for a daily and weekly
//---------------------------------------------------------------------------------------

$sql = "SELECT cre_id, cre_dateDebut, cre_dateFin, cre_repeat FROM Creneau";

$dateNow = date_create_from_format('Y-m-d H:i:s', $maintenant);
$today = $dateNow->format('d-m-Y');
...
if ($result = mysqli_query($con, $sql))
{
    ...
    {  
        ...
        $cre_id = $row->cre_id;         
        $cre_dateDebut  = date_create($row->cre_dateDebut);
        $cre_dateFin  = date_create($row->cre_dateFin);
        $cre_repeat  = $row->cre_repeat;


        $dayDiff = dateDifference($today, $cre_dateFin->format('d-m-Y'));
		
		// none => cre_repeat = 0
		// daily => cre_repeat = 1
		// weekly => cre_repeat = 2	
		
        if ($dayDiff > 0 && $cre_repeat == 0) {
         
          $sql = "DELETE FROM Creneau WHERE cre_id = " . $cre_id;       
 		  $result1 = mysqli_query($con, $sql);
 		        
        } else if (($dayDiff > 0 && $cre_repeat == 1) || ($dayDiff > 6 && $cre_repeat == 2)) {
         
		  date_date_set($cre_dateDebut, date("Y"), date("m"), date("d"));
		  date_date_set($cre_dateFin, date("Y"), date("m"), date("d"));

          $sql = "UPDATE Creneau SET cre_dateDebut='" . $cre_dateDebut->format('Y-m-d H:i:s') .
           "', cre_dateFin='" . $cre_dateFin->format('Y-m-d H:i:s') . "' WHERE cre_id = " . $cre_id;
...
if ($result = mysqli_query($con, $sql))
{
    ...
    {   
        ...
        if ($result1 = mysqli_query($con, $sql))
        {
            ...
            if ($flgOK == 0) {
                $sql = "UPDATE Product SET prod_hidden = 0 , prod_closed = 0,  prod_oth_user = 0 WHERE prod_id = '" . mysqli_real_escape_string($con, $prodId) . "'";
