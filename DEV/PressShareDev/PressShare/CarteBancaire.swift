...
class MDBCarteB {
    ...
    /*
     ...
     // Create Data from request
     let request = NSMutableURLRequest(url: URL(string: "\(CommunRequest.sharedInstance.urlServer)/api_updatepass.php")!)
     // set Request Type
     ...
     // Create your request string with parameter name as defined in PHP file
     let body: String = "prod_by_user=\(produit.prod_by_user)&prod_date=\(produit.prod_date)&prod_nom=\(produit.prod_nom)&prod_prix=\(produit.prod_prix)&prod_by_cat=\(produit.prod_by_cat)&prod_latitude=\(produit.prod_latitude)&prod_longitude=\(produit.prod_longitude)&prod_mapString=\(produit.prod_mapString)&prod_comment=\(produit.prod_comment)&prod_imageUrl=\(produit.prod_imageUrl)"
     // Create Data from request
     let request = NSMutableURLRequest(url: URL(string: "\(CommunRequest.sharedInstance.urlServer)/api_addproduit.php")!)
     // set Request Type
     request.httpMethod = "POST"
     // Set content-type
     request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "content-type")
