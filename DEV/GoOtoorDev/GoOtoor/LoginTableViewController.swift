...
class LoginTableViewController : UITableViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var IBBiometricID: UIImageView!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        IBUser.layer.addSublayer(MyTools.sharedInstance.createLine(frame: IBUser.frame))
        
        IBPasswordLabel.text = translate.message("password")
        IBPasswordLabel.textColor = UIColor.init(hexString: config.colorAppLabel)
        
        IBPassword.layer.addSublayer(MyTools.sharedInstance.createLine(frame: IBPassword.frame))
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            IBBiometricID.isHidden = false
            if #available(iOS 11.0, *) {
                switch context.biometryType {
                case .touchID:
                    IBBiometricID.image = #imageLiteral(resourceName: "fingerPrint")
                    
                case .faceID:
                    IBBiometricID.image = #imageLiteral(resourceName: "faceID")
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if (NSKeyedUnarchiver.unarchiveObject(withFile: filePath) as? Bool) == nil  {
            ...
            MyTools.sharedInstance.showHelp("Tuto_Presentation", self)
    ...
    func callAuthFacebook() {
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            ...
            if #available(iOS 11.0, *) {
                switch context.biometryType {
                case .touchID:
                    reason = translate.message("touchIDInf")
                    errorString = translate.message("touchIDFai")
                case .faceID:
                    reason = translate.message("faceIDInf")
                    errorString = translate.message("faceIDFai")
                case .none: print("No Biometric support")
                }
            } else {
                reason = translate.message("touchIDInf")
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason, reply:
                {(succes, error) in
                    ...
                    if succes {
                        ...
                        MDBUser.sharedInstance.AuthentiFacebook(self.config) { (success, userArray, errorString) in
            ...
            displayAlert(translate.message("error"), mess: translate.message("touchIDAva"))
            self.IBActivity.isHidden = true
            self.IBActivity.stopAnimating()
            self.setUIEnabled(true)
    ...
    func handleTap(sender: UITapGestureRecognizer) {
        if sender.state == .ended {
            ...
            if indexPath?.row == 0 && indexPath?.section == 0 {
                ...
                if IBBiometricID.isHidden == false {
                    
                    let cell = tableView.cellForRow(at: indexPath!)
                    let zy = location.y - (cell?.frame.origin.y)!
                    
                    let xw2 = IBBiometricID.frame.origin.x + IBBiometricID.frame.size.width
                    let yh2 = IBBiometricID.frame.origin.y + IBBiometricID.frame.size.height
                    
                    if zx <= xw2 && zx >= IBBiometricID.frame.origin.x && zy  <= yh2 && zy >= IBBiometricID.frame.origin.y {
                        ...
                        if config.user_email != "" {
                            ...
                            tableView.endEditing(true)
                            displayAlert(translate.message("error"), mess: translate.message("firstCon"))
                        }
