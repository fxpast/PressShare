...
class CommunRequest {
    ...
    let urlServer = "http://pressshare.gootoor.com"
    ...
    func responseRequest(_ data:Data?, _ response:URLResponse, _ error:Error?, completionHdler:(_ success:Bool, _ result:Any?, _ errorString:String?) -> Void) {
        ...
        guard let statusCode = (response as? HTTPURLResponse)?.statusCode , statusCode >= 200 && statusCode <= 299 else {
            completionHdler(false, nil, "\(translate.message("errorRequestReturn")) \(MyTools.sharedInstance.statusCode(((response as? HTTPURLResponse)?.statusCode)!))")
            return
