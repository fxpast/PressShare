...
class SubscriptViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource  {
    ...
    override func viewDidLoad() {
        ...
        IBBalance.text = MyTools.sharedInstance.formatedAmount(config.balance!)
    ...
    @IBAction func actionHelp(_ sender: Any) {
        ...
        //action info
        MyTools.sharedInstance.showHelp("SubscriptViewController", self)
                ...
                MDBPressOperation.sharedInstance.setAddOperation(operation, completionHandlerAddOp: {(success, errorString) in
                    ...
                    if success {
                        ...
                        MyTools.sharedInstance.performUIUpdatesOnMain {
                            ...
                            self.IBBalance.text = MyTools.sharedInstance.formatedAmount(self.config.balance!)
    ...
    @IBAction func actionWithdrawal(_ sender: Any) {
        ...
        let actionValider = UIAlertAction(title: translate.message("done"), style: .destructive, handler: { (action) in
            ...
            guard let amount = MyTools.sharedInstance.formatedAmount(self.IBWithdrawal.text!) else {
            ...
            guard let balance = MyTools.sharedInstance.formatedAmount(finalValue) else {
                MyTools.sharedInstance.performUIUpdatesOnMain {
    ...
    @IBAction func actionDeposit(_ sender: Any) {
        ...
        guard let amount = MyTools.sharedInstance.formatedAmount(self.IBDeposit.text!) else {
    ...
    private func checkDeposit(_ amount: Double) -> Bool {
        ...
        if aCard == nil {
            ...
            resultat = false
            alertController = UIAlertController(title: translate.message("creditCard"), message: translate.message("addCreditCard"), preferredStyle: .alert)
    ...
    @IBAction func actionSubUnsub(_ sender: Any) {
        ...
        let actionValider = UIAlertAction(title: translate.message("done"), style: .destructive, handler: { (action) in
            ...
            if self.config.level <= 0 && self.config.balance == 0 {
                ...
                if (self.checkDeposit(self.config.subscriptAmount!)) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        aAmount.text = MyTools.sharedInstance.formatedAmount(operation.op_amount)
